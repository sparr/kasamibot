RoomPosition.prototype.getClosestSpawn = function getClosestSpawn(): Spawn {
    return this.findClosestByRange(FIND_MY_SPAWNS) as Spawn;
};

RoomPosition.prototype.hasFreeSpaceAround = function hasFreeSpaceAround(): boolean {
    for (let x = -1; x < 2; x++) {
        for (let y = -1; y < 2; y++) {
...
RoomPosition.prototype.getFreeSpaceAround = function getFreeSpaceAround(): number {
    let c = 0;
    for (let x = -1; x < 2; x++) {
        for (let y = -1; y < 2; y++) {
        let position = new RoomPosition(this.x + x, this.y + y, this.roomName);
...
RoomPosition.prototype.hasBuildingType = function hasBuildingType(structureType: string): boolean  {
...
RoomPosition.prototype.getPositionInDirection = function getPositionInDirection(direction: number): RoomPosition  {
    switch (direction) {
        case TOP:
